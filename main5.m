...
imgPath2 = '../../Sonnenlicht/2/';
fileNames2 = dir([imgPath2 '*.jpg']);
M = length(fileNames2);
fileNo2 = [];

for i = 1:M
    temp = regexp(fileNames2(i).name(1:end-4),'_','split');
    fileNo2 = [fileNo2;str2double(temp(2))*1000+str2double(temp(3))];
end
[~,id] = sort(fileNo2);


thFeature=2000;
dir_bg = './output/method5/bg/';
dir_fg = './output/method5/fg/';
...
for i = 1:M
    im0 = im2double(imread([imgPath2 fileNames2(id(i)).name]));
    im = rgb2gray(im0);
    if size(im,1) ~= imSize(1) & size(im,2) ~= imSize(2)
        im = imresize(im,imSize);
    end
    ...
    if BeM(i)>thFeature
        ...
        imwrite(im0,[dir_fg  fileNames2(id(i)).name]);
    else
        title([num2str(i) ': Frei  s:' num2str(BeM(i))]);
        imwrite(im0,[dir_bg  fileNames2(id(i)).name]);
    end
    drawnow
%     pause(0.2)
end
